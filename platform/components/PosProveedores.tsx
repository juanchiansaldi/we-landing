...
type Row = {
  ...
  email: string; notas: string; activo: boolean; count: number;
};
const EMPTY: Row = { id: "", nombre: "", cuit: "", contacto: "", telefono: "", email: "", notas: "", activo: true, count: 0 };
  ...
  return (
    <div className="pos-wrap">
      ...
      <div className="admin-table">
        <div className="admin-tr admin-th" style={{ gridTemplateColumns: "1.4fr 1fr 1fr .6fr 1fr" }}>
        ...
        {proveedores.map((p) => (
          <div className="admin-tr" key={p.id} style={{ gridTemplateColumns: "1.4fr 1fr 1fr .6fr 1fr", opacity: p.activo ? 1 : 0.5 }}>
            <span className="admin-name"><b>{p.nombre}</b>{p.contacto && <em>{p.contacto}</em>}{!p.activo && <em style={{ color: "var(--gray)" }}>inactivo</em>}</span>
            <span>{p.cuit || "—"}</span>
      ...
      {editing && (
        <div className="admin-modal" onClick={() => !pending && setEditing(null)}>
          <form className="admin-card admin-editor" onClick={(e) => e.stopPropagation()} onSubmit={onSave}>
            ...
            <label>Nombre *<input name="nombre" defaultValue={editing.nombre} required autoFocus /></label>
