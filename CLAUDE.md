...
- **Auth**: el panel `/admin` y el POS se protegen con `isAuthed()` (cookie firmada, password en `ADMIN_PASSWORD`). Login de clientes de la tienda = Supabase Auth. Usuarios POS por vendedor = Fase 7 (pendiente).
...
`/admin` (inicio + gráfico) · `/admin/productos` · `/admin/categorias` · `/admin/proveedores` · `/admin/pedidos` (con marcar pagado + comprobantes) · `/admin/cupones` · `/admin/reportes`. Rutas POS futuras: Vender, Stock, Caja.

## Fases del POS
1 Catálogo+Import ✅ · 2 Vender (POS) ▶ en curso · 3 Stock · 4 Reportes ✅ · 5 Compras · 6 Clientes/CC · 7 Usuarios/roles · 8 Caja · 9 Combos/kits (con costo combinado + margen %) · 10 Facturación ARCA (futuro, NO construir hasta avisar; hoy tickets internos no fiscales).
